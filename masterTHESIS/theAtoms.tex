...
In order to solve for the dynamics, we must apply the rotating wave approximation. This means that we rewrite $\mathbf{E}$ from Eq.\,\ref{eqn:Efield} as 
\begin{multline}
\label{Ebreakdown}
...
\end{multline}
and then we will assume that only the slowly-oscillating terms will make non-zero contributions to the dynamics of the system. In other words, we now 
