...
We may well employ the rotating wave approximation (RWA). Let $H_0$ be the Hamiltonian of our unperturbed system (the atom) and let $V$ be the perturbation (which we will use to model the electric field). 
...
Now, we can move to the interaction picture in the standard way. Let $|\alpha\rangle$ be the eigenstate of $H_0$, the unperturbed Hamiltonian$\begin{smallmatrix}a&b\\c&d\end{smallmatrix}$: 
...
%also, IDK what to do about the the Nucleur spin numbers of the upper state. 
...
$F=4$ P$_{3/2}$ ($5p$)  & Ground state & label \\ \hline
$F=5$ P$_{3/2}$ ($5p$) & Ground state & label \\ \hline
\end{tabular}
\end{center}
