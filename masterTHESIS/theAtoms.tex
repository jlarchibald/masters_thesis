...
+
\frac{1}{2}e^{i(\omega_e-\omega_i+\omega_2)t+\phi_2}\langle e|\mathbf{d}\cdot\mathbf{E}_2| i\rangle + \frac{1}{2}e^{i(\omega_e-\omega_i-\omega_2)t-\phi_2}\langle e|\mathbf{d}\cdot\mathbf{E}_2| i\rangle +
\ldots
\end{multline}
The rotating wave approximation is to assume that only the slowly-oscillating terms will make non-zero contributions to the dynamics of the system\cite{Young1997363}. Thus, we can write 
...
So, for example, if we look at the $\langle i |V_I|e\rangle$, we would have some terms with 
%\begin{align}
%\langle i|V_I|e\rangle &= \langle i|e^{i H_0 t/\hbar} (\mathbf{d}\cdot\mathbf{E}) e^{-iH_0 t/\hbar}|e\rangle\\
%&=  \frac{1}{2}e^{i (\omega_i-\omega_e) t} 
%\left(\langle i|\mathbf{d}\cdot\mathbf{E_1}|e\rangle 
%( e^{i (\omega_1 t + \phi_1)}+ e^{-i (\omega_1 t + \phi_1)}) +
%\langle i|\mathbf{d}\cdot\mathbf{E_2}|e\rangle 
%( e^{i (\omega_2 t + \phi_2)}+ e^{-i (\omega_2 t + \phi_2)})\right)\\
%&=  \frac{1}{2}e^{i (\omega_i-\omega_e+\omega_1) t + \phi_1} 
%\left(\langle i|\mathbf{d}\cdot\mathbf{E_1}|e\rangle 
%( e^{i (\omega_1 t + \phi_1)}+ 
%e^{-i (\omega_1 t + \phi_1)}) +\langle i|\mathbf{d}\cdot\mathbf{E_2}|e\rangle ( e^{i (\omega_2 t + \phi_2)}+ e^{-i (\omega_2 t + \phi_2)})\right)\\
%\end{align}


We can write $V_I$ explicitly using Eqs.\,\eqref{VSchrod}, \eqref{intTransforms}, \eqref{expH0}, and \eqref{Ebreakdown}:
\begin{equation}
V_I=
\begin{bmatrix}
0 & 0 & \langle e |\mathbf{d}\cdot\mathbf{E}|i\rangle e^{i(-\omega_e+\omega_i)t}\\
