...
\end{equation}
Refs.\,\cite{sakurai} and \cite{merzbacher} explain this. Note that the basis $|e\rangle$,$|g\rangle$,$|i\rangle$, we can write $\exp(iH_0t/\hbar)$ as 
\begin{equation}
\label{expH0}
...
\end{equation}
where $c_e,c_g,c_i$ are slow-changing coefficients.
...
\end{multline}
Basically, the rotating wave approximation is that we ignore the off-resonant terms. So, for example, if I wanted to look at $_I\langle e |V_I|e\rangle_I$, 
