...
\end{equation}
These equations follow the conventions selected in Ref.\,\cite{Young1997363}. 
...
These results are also quoted from Ref.\,\cite{Young1997363}, which gives a reasonably good explanation of the methods used.
...
H=H_0+V.
\end{equation}
Here, $H_0$ is the Hamiltonian of the atom without the influence of the radiation from the laser. It is written in terms of three states ($|e\rangle$,$|g\rangle$,and $|i\rangle$). The symbol $V$ represents the interaction between the atom and the laser: 
\begin{align}
H_0&=
...
We can also get to the two state equations of motion using classic perturbation theory. If we take the standard time-dependent perturbation to second order, we get:
